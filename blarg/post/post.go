...
func GetAllTags(context appengine.Context) ([]string, []int, error) {
	...
	if len(rawtags) > 1 {
		return nil, nil, errors.New("not only one tagcounts struct")
	}
	if len(rawtags) < 1 {
		return make([]string, 0), make([]int, 0), nil
	}
	if len(rawtags[0].Name) != len(rawtags[0].Count) {
		return nil, nil, errors.New("tagcounts Name length doesn't match Counts length")
	}
